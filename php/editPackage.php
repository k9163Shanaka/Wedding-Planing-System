...
<?php 
	...
	//edit package
	if(isset($_GET["edit_pack_id"])){
		
		$id = $_GET["edit_pack_id"];
	...
	if(isset($_POST['submit'])){
		...
		$req_fields = array('pack_id', 'package_category', 'package_name', 'package_code', 'price', 'description');
		...
		$sql = "SELECT * FROM package WHERE pack_code = '{$pack_code}' AND pack_id != {$id} LIMIT 1";
		...
		if (empty($errors)) {
			...
			$sql = "UPDATE package 
					SET pack_code = '{$pack_code}', pack_name = '{$pack_name}', pack_title = '{$pack_title}', pack_price = '{$pack_price}', pack_detail = '{$pack_detail}', pack_img = '{$pack_img}' 
					WHERE pack_id = {$id} 
					LIMIT 1";	
...
<html>
	...
	<form action="editPackage.php"  method="post" class="pack_form" enctype="multipart/form-data">
		...
		<input type="submit" class="input_field_sbtn" id="sbtn" name="submit" value="Save" disabled>
